...
== Configuration
=== configmap
. Set your tenant's id and URL in *k8s-config.yaml*

    tenant_id: <tenant-id> ## for instance, fdg3423
    tenant_url: <url> ## e.g. https://fdg3423.sprint.dynatracelabs.com

. Define instrumentation (OneAgent or Otel) for the microservices in *k8s-config.yaml*
...
. Put tokens in the secrets, *k8s-secret.yaml* file
